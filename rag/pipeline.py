...
class SharedEmbeddingModel:
    ...
    def initialize_model(
        self, qdrant_client, model_name: str = "BAAI/bge-base-en-v1.5"
    ):
        with self._lock:
            ...
            try:
                ...
                initialize_collection_if_needed(qdrant_client, vector_size)
...
def initialize_collection_if_needed(client, vector_size: int):
    if not client.collection_exists(COLLECTION_NAME):
        ...
        try:
            ...
            if existing_size != vector_size:
                print(
                    f"Warning: Existing collection has vector size {existing_size}, but model produces {vector_size}"
                )
                print(
                    f"Deleting existing collection and recreating with correct size..."
                )
...
class MergedRAGWorker:
            ...
            with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
                ...
                for chunk in lazy_read(f, chunk_size_kb=4):
                    ...
                    for chunk_to_yield in chunklets:
                        if (
                            len(chunk_to_yield.strip()) > 5
                        ):  # Yield only non-empty chunks
                            if len(chunk_to_yield.encode("utf-8")) > 1024:
                                yield from self._semantic_chunking_logic(
                                    chunk_to_yield, filename, chunk_id
                                )
                                chunk_id += 100
                            else:
    ...
    def _semantic_chunking_logic(
        self, chunk: str, filename: str, chunk_id: int
        ...
        for j, breakpoint in enumerate(breakpoint_distance_threshold):
            indices_above_thresh = [
                i for i, x in enumerate(distances) if x > breakpoint
            ]
            ...
            if len(indices_above_thresh) == 0:
                if len(chunk.encode("utf-8")) <= self.chunk_size_bytes:
                    ...
                    for chunk_to_yield in self._split_large_text(
            ...
            for i in range(len(indices_above_thresh)):
                ...
                if len(chunk_to_yield.encode("utf-8")) <= self.chunk_size_bytes:
                    ...
                    for chunk in self._split_large_text(
                        chunk_to_yield, self.chunk_size_bytes
                    ):
                        yield (chunk, filename, chunk_id, j)
                        chunk_id += 1
    ...
    def _extract_json_chunks(
        self, data: Any, max_size: int, current_path: str = ""
            ...
            for key, value in data.items():
                ...
                if chunk_size <= max_size:
                    ...
                    if isinstance(value, (dict, list)):
                        context = f"{new_path}: "
                        yield from self._extract_json_chunks(
                            value, max_size - len(context.encode("utf-8")), new_path
                        )
                    else:
    ...
    def _embed_chunks(
        ...
        try:
            ...
            if not texts or any(not text.strip() for text in texts):
                print(
                    f"Worker {self.worker_id}: Warning - some texts are empty or whitespace-only"
                )
                valid_chunks = [
                    (text, filename, chunk_id, threshold)
                    for text, filename, chunk_id, threshold in chunks
                    if text.strip()
                ]
                if not valid_chunks:
            ...
            try:
                ...
                if not embeddings or len(embeddings) != len(texts):
                    print(
                        f"Worker {self.worker_id}: Embedding count mismatch - expected {len(texts)}, got {len(embeddings) if embeddings else 0}"
                    )
                    return []
    ...
    def _store_embeddings(
        self, embeddings_with_payloads: List[Tuple[List[float], Dict[str, Any]]]
    ) -> int:
        ...
        try:
            ...
            initialize_collection_if_needed(self.qdrant_client, vector_size)
            ...
            for embedding, payload in embeddings_with_payloads:
                ...
                if len(embedding) != vector_size:
                    print(
                        f"Worker {self.worker_id}: Warning - embedding size mismatch: expected {vector_size}, got {len(embedding)}"
                    )
                    continue
